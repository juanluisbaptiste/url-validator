...
def isDomainNameValid(name):
    ...
    """
    # TODO: Works but accepts hostnames with a name of at least 3 characters
...
def writeInvalidFile(filename):
    ...
    for url in invalid_urls:
        content += url + "," + repr(invalid_urls[url]) + "\n"
...
def start(args):
    ...
    if args.test_urls:
        ...
        print "Done."
        testUrlsStats()
    print "Total of parsed url's: " + repr(len(valid_urls) + len(invalid_urls))
...
def parsingFileStats(args):
    ...
    print "Parsing a total of " + repr(url_counter) + " url's...\n"
...
def testUrlsStats():
    ...
    """
    print "\nFound " + repr(invalid_urls_counter) + " not responding url's"
    print "\nResults:\n"
    print "Valid url's: " + repr(len(valid_urls))
    print "Total invalid url's: " + repr(len(invalid_urls))
